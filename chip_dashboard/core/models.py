...
class Exchange(TimeStampedModel):
    ...
    version = models.CharField(max_length=50, blank=True, null=True, help_text="Optional version identifier for the exchange")
    is_active = models.BooleanField(default=True, help_text="Whether this exchange is active and can be used")
    
    class Meta:
        ordering = ['name']
        constraints = [
            models.UniqueConstraint(
                fields=['name', 'code', 'version'],
                name='unique_exchange_name_code_version',
            )
        ]
    
    def clean(self):
        """
        Validate that exchange name+code+version combination is unique (case-insensitive).
        Allows same name+code with different versions, but prevents duplicate name+code+version.
        """
        from django.core.exceptions import ValidationError
        
        if self.name:
            # Normalize code and version to empty string if None for comparison
            code_normalized = (self.code or '').strip().lower()
            version_normalized = (self.version or '').strip().lower()
            ...
            # Check each existing exchange for matching code+version
            for ex in existing:
                ...
                if existing_code_normalized == code_normalized and existing_version_normalized == version_normalized:
                    ...
                    raise ValidationError(
                        f"Exchange '{self.name}{code_display}{version_display}' already exists. "
                        f"Same name+code with different version is allowed, but duplicate name+code+version is not."
                    )
